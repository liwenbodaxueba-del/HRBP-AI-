...
check("台账加行缺必填422(弹窗入库)", r.status_code == 422, str(j(r)))
r = client.post("/api/ledger/row", json={"fields": {}, "draft": True}, headers=H)
check("直接加空白行(draft跳过必填)200", r.status_code == 200 and j(r).get("id"), str(j(r)))
client.delete(f"/api/ledger/row/{j(r)['id']}", headers=H)  # 清场
...
check("看板1预算联动看板2(demo期初550生效)", b["metrics"]["budget"]["vals"][0] == 550)
r = client.post("/api/board/2026/branch", json={"sec": "预算当量·其中", "name": "联动测试", "sign": "+"}, headers=H)
bq = [x for x in j(r)["branches"] if x["name"] == "联动测试"][0]["id"]
r = client.post("/api/board/2026/cell", json={"metric": f"branch:{bq}", "month": 8, "value": 4, "note": ""}, headers=H)
check("看板2其中调整→看板1预算8月=554", j(r)["metrics"]["budget"]["vals"][7] == 554, str(j(r)["metrics"]["budget"]["vals"][7]))
client.delete(f"/api/board/2026/branch/{bq}", headers=H)  # 清场
...
check("导出CSV", r.status_code == 200 and "实际/预估期末在岗" in r.text)
