...
CANON_PROJECTS = [
    ...
    ("o_sys", "总流出（−）", "待流出 · 系统明确", "HR数仓（待接）", "src", 0, 1, 1),
    ("o_nat", "总流出（−）", "待流出 · 自然流失预估", "运算派生", "calc", 0, 1, 1),
    ("o_bp", "总流出（−）", "待流出 · 已明确非系统（BP）", "BP 手填", "bp", 1, 1, 1),
    ("o_act", "总流出（−）", "待流出 · 主动动作（BP）（调节项）", "BP 手填", "bp", 1, 1, 1),
    ("i_soc", "总流入（＋）", "待流入 · 社招", "分列求和", "calc", 1, 1, 1),
    ...
    ("camp", "总流入（＋）", "待流入 · 校招", "分列求和", "calc", 1, 1, 1),
...
EXTRA_METRICS = {  # 看板2 预算过程基线（不在看板1 行内，但可存取/编辑/导入）
    ...
    "soc_join": "社招·已入职（系统接口·HR数仓，非3.1台账）",  # 系统读取·独立于 ai_soc(招聘系统)
